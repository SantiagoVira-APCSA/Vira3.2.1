public class CatHouse {
    ...
    public void rollCall() {
        if (cat1.getAge() != cat2.getAge()) {
            Cat oldest = cat1.getAge() > cat2.getAge() ? cat1 : cat2;
            Cat youngest = cat1.getAge() > cat2.getAge() ? cat2 : cat1;

            oldest.introduce();
            youngest.introduce();
        } else {
            if (cat1.getName().compareTo(cat2.getName()) != 0) {
                Cat first = cat1.getName().compareTo(cat2.getName()) < 0 ? cat1 : cat2;
                Cat second = cat1.getName().compareTo(cat2.getName()) < 0 ? cat2 : cat1;
